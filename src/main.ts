...
const polarToCartesian = (polar: PolarCoordinate): PointData => {
  return {
...
const sphericalToCartesian3D = (spherical: SphericalCoordinate): Point3D => {
...
const project3DTo2D = (point3D: Point3D, distance: number = 400): PointData => {
  // 透視投影（簡単な正射投影として処理）
...
class Point {
  private sphericalCoord: SphericalCoordinate = { radius: 0, phi: 0, theta: 0 };
  private position3D: Point3D = { x: 0, y: 0, z: 0 };
  private position: PointData = { x: 0, y: 0 };
  private previousPosition: PointData | null = null;
  private p: any;
  private noiseOffset: number = 0;
  private rotationSpeed: { phi: number; theta: number } = { phi: 0, theta: 0 };

  constructor(p: any, startTime: number = 0) {
    this.p = p;
    this.init(startTime);
  }

  init(startTime: number = 0) {
...
const sketch = (p: any) => {
  let circles: any[] = [];
  let point: Point;
  let worldSpeed: number = 5.0;
  let frameCount: number = 0;
  let displayStartTime: number = 0; // 画面表示開始時の実時間（秒）
  ...
  p.draw = () => {
    ...
    p.text(
      `Time: ${displayElapsedSeconds.toFixed(1)}s`,
      leftEdge,
      topEdge + 25,
    );
    p.textSize(12);
